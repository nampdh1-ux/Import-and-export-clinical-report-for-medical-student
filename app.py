...
# Thanh công cụ trên cùng kết hợp Chọn mẫu bệnh án & Máy tính lâm sàng
c_top_radio, c_top_calc = st.columns([3, 1])
with c_top_radio:
    loai_benh_an = st.radio("📌 **LỰA CHỌN MẪU BỆNH ÁN:**", ["Nội khoa / Tiền phẫu", "Hậu phẫu"], horizontal=True, key="loai_benh_an")
with c_top_calc:
    st.write("")
    ui_clinical_calculators()

# Khai báo Dictionary lưu trữ ảnh toàn cục
uploaded_imgs = {}

# ==============================================================================
# MÁY TÍNH LÂM SÀNG NHÚNG (EMBEDDED CLINICAL CALCULATORS)
# ==============================================================================
def ui_clinical_calculators():
    with st.popover("🧮 Máy tính lâm sàng", use_container_width=True):
        st.markdown("#### 🧮 Tra cứu & Tính điểm Lâm sàng")
        calc_type = st.selectbox(
            "Chọn thang điểm:",
            [
                "1. Cockcroft-Gault (Chức năng thận & CrCl)",
                "2. Thang điểm Glasgow (GCS - Tri giác)",
                "3. Thang điểm Alvarado (Viêm ruột thừa cấp)",
                "4. Thang điểm Caprini (Nguy cơ DVT/VTE sau mổ)",
                "5. Phân độ biến chứng Clavien-Dindo (Hậu phẫu)",
                "6. Tỷ số Sốc (Shock Index)"
            ],
            key="calc_selector_choice"
        )
        st.divider()

        # 1. COCKCROFT-GAULT
        if "Cockcroft-Gault" in calc_type:
            st.caption("Ước tính độ thanh thải Creatinine (CrCl) để chỉnh liều kháng sinh & thuốc.")
            c_cg1, c_cg2 = st.columns(2)
            with c_cg1:
                cg_tuoi = st.number_input("Tuổi", value=int(st.session_state.get("tuoi", 45)), min_value=1, max_value=120, key="cg_tuoi")
                cg_can_nang = st.number_input("Cân nặng (kg)", value=float(st.session_state.get("sh_can_nang", 60.0) or 60.0), min_value=10.0, step=0.5, key="cg_cn")
            with c_cg2:
                cg_gioi = st.selectbox("Giới tính", ["Nam", "Nữ"], index=0 if st.session_state.get("gioi_tinh") == "Nam" else 1, key="cg_gioi")
                cg_creat = st.number_input("Creatinine máu (µmol/L)", value=80.0, min_value=10.0, max_value=2000.0, step=5.0, key="cg_creat")

            if cg_creat > 0:
                # Công thức chuẩn µmol/L: CrCl = [(140 - tuổi) * P (kg)] / (72 * (Scr / 88.4)) = [(140 - tuổi) * P * 1.23] / Scr (nếu là nữ nhân 0.85)
                factor = 1.04 if cg_gioi == "Nữ" else 1.23
                crcl = round(((140 - cg_tuoi) * cg_can_nang * factor) / cg_creat, 1)
                st.metric("Độ thanh thải CrCl:", f"{crcl} mL/phút")
                if crcl >= 90:
                    danh_gia = "Chức năng thận bình thường (G1)"
                elif crcl >= 60:
                    danh_gia = "Giảm nhẹ chức năng thận (G2)"
                elif crcl >= 30:
                    danh_gia = "Suy thận mức độ trung bình (G3) - Cần chỉnh liều nhiều thuốc"
                elif crcl >= 15:
                    danh_gia = "Suy thận nặng (G4) - Tránh các thuốc độc thận"
                else:
                    danh_gia = "Suy thận giai đoạn cuối (G5)"
                st.info(f"**Đánh giá:** {danh_gia}")

                if st.button("📋 Chèn vào mục Biện luận CLS", key="btn_chen_crcl"):
                    chen_txt = f"- Độ thanh thải Creatinine (Cockcroft-Gault): CrCl = {crcl} mL/phút ({danh_gia})."
                    add_symptom_to_field("bien_luan", chen_txt)
                    st.toast("Đã chèn kết quả CrCl vào Biện luận!", icon="✅")

        # 2. GLASGOW (GCS)
        elif "Glasgow" in calc_type:
            st.caption("Đánh giá độ hôn mê / tri giác (3 - 15 điểm).")
            gcs_e = st.selectbox("Mắt (Eye - E):", [
                "4 - Mở mắt tự nhiên",
                "3 - Mở mắt khi gọi / nghe tiếng nói",
                "2 - Mở mắt khi kích thích đau",
                "1 - Không mở mắt"
            ], key="gcs_e_val")
            gcs_v = st.selectbox("Lời nói (Verbal - V):", [
                "5 - Trả lời đúng, định hướng tốt",
                "4 - Trả lời lẫn lộn, lú lẫn",
                "3 - Nói từ ngữ không thích hợp",
                "2 - Kêu rên, phát âm khó hiểu",
                "1 - Không phát âm"
            ], key="gcs_v_val")
            gcs_m = st.selectbox("Vận động (Motor - M):", [
                "6 - Làm theo y lệnh",
                "5 - Định khu được vị trí đau",
                "4 - Rụt chi lại khi kích thích đau",
                "3 - Co cứng mất vỏ (gấp chi bất thường)",
                "2 - Duỗi cứng mất não (duỗi chi bất thường)",
                "1 - Không cử động"
            ], key="gcs_m_val")

            diem_gcs = int(gcs_e[0]) + int(gcs_v[0]) + int(gcs_m[0])
            st.metric("Tổng điểm Glasgow:", f"{diem_gcs} / 15 điểm")
            if diem_gcs == 15:
                dg_gcs = "Bệnh nhân tỉnh táo hoàn toàn"
            elif diem_gcs >= 13:
                dg_gcs = "Rối loạn tri giác mức độ nhẹ"
            elif diem_gcs >= 9:
                dg_gcs = "Hôn mê mức độ trung bình"
            else:
                dg_gcs = "Hôn mê sâu (Đe dọa đường thở, cân nhắc đặt NKQ)"
            st.info(f"**Đánh giá:** {dg_gcs}")

            if st.button("📋 Chèn vào Khám toàn thân", key="btn_chen_gcs"):
                chen_txt = f"Glasgow {diem_gcs} điểm (E{gcs_e[0]}V{gcs_v[0]}M{gcs_m[0]} - {dg_gcs})"
                add_symptom_to_field("kham_toan_than", chen_txt)
                st.toast("Đã chèn kết quả GCS!", icon="✅")

        # 3. ALVARADO (MANTRELS)
        elif "Alvarado" in calc_type:
            st.caption("Thang điểm chẩn đoán viêm ruột thừa cấp (Tổng 10 điểm).")
            c1, c2 = st.columns(2)
            with c1:
                al_m = st.checkbox("Đau di chuyển hố chậu phải (Migration) (+1)", key="al_m")
                al_a = st.checkbox("Chán ăn (Anorexia) (+1)", key="al_a")
                al_n = st.checkbox("Buồn nôn / Nôn (Nausea/Vomiting) (+1)", key="al_n")
                al_t = st.checkbox("Đau tức hố chậu phải (RLQ Tenderness) (+2)", value=True, key="al_t")
            with c2:
                al_r = st.checkbox("Phản ứng dội Blumberg / Cảm ứng phúc mạc (+1)", key="al_r")
                al_f = st.checkbox("Sốt nhẹ ≥ 37.3°C (+1)", key="al_f")
                al_l = st.checkbox("Bạch cầu tăng > 10.0 G/L (+2)", key="al_l")
                al_s = st.checkbox("Tỷ lệ Neutrophil chuyển trái > 75% (+1)", key="al_s")

            diem_alv = (1 if al_m else 0) + (1 if al_a else 0) + (1 if al_n else 0) + (2 if al_t else 0) + \
                       (1 if al_r else 0) + (1 if al_f else 0) + (2 if al_l else 0) + (1 if al_s else 0)
            st.metric("Điểm Alvarado:", f"{diem_alv} / 10 điểm")
            if diem_alv <= 4:
                dg_alv = "Ít nghĩ đến viêm ruột thừa (Có thể tìm nguyên nhân khác)"
            elif diem_alv <= 6:
                dg_alv = "Nghi ngờ viêm ruột thừa (Cần siêu âm ổ bụng / CT theo dõi)"
            elif diem_alv <= 8:
                dg_alv = "Khả năng cao viêm ruột thừa cấp (Chỉ định hội chẩn ngoại khoa)"
            else:
                dg_alv = "Gần như chắc chắn viêm ruột thừa cấp (Chỉ định phẫu thuật cấp cứu)"
            st.info(f"**Đánh giá:** {dg_alv}")

            if st.button("📋 Chèn vào Biện luận chẩn đoán", key="btn_chen_alv"):
                chen_txt = f"- Thang điểm Alvarado: {diem_alv}/10 điểm ({dg_alv})."
                add_symptom_to_field("bien_luan", chen_txt)
                st.toast("Đã chèn kết quả Alvarado!", icon="✅")

        # 4. CAPRINI SCORE (DVT RISK)
        elif "Caprini" in calc_type:
            st.caption("Đánh giá nguy cơ thuyên tắc huyết khối tĩnh mạch sau phẫu thuật.")
            cap_age = st.radio("Độ tuổi:", ["Dưới 41 (0đ)", "41 - 60 (+1đ)", "61 - 74 (+2đ)", "≥ 75 tuổi (+3đ)"], key="cap_age")
            diem_age = 0 if "Dưới" in cap_age else 1 if "41" in cap_age else 2 if "61" in cap_age else 3

            c_c1, c_c2 = st.columns(2)
            with c_c1:
                cp_surg = st.checkbox("Mổ lớn kéo dài > 45 phút / Nội soi > 45 phút (+2đ)", key="cp_surg")
                cp_bed = st.checkbox("Bệnh nhân nằm bất động tại giường > 72h (+2đ)", key="cp_bed")
                cp_malig = st.checkbox("Bệnh nhân có bệnh lý ung thư tiến triển (+2đ)", key="cp_malig")
            with c_c2:
                cp_bmi = st.checkbox("BMI > 25 kg/m² (+1đ)", key="cp_bmi")
                cp_dvt_hist = st.checkbox("Tiền sử cá nhân/gia đình DVT/PE (+3đ)", key="cp_dvt_hist")
                cp_sepsis = st.checkbox("Nhiễm khuẩn huyết / Viêm phổi sau mổ (+1đ)", key="cp_sepsis")

            diem_caprini = diem_age + (2 if cp_surg else 0) + (2 if cp_bed else 0) + (2 if cp_malig else 0) + \
                           (1 if cp_bmi else 0) + (3 if cp_dvt_hist else 0) + (1 if cp_sepsis else 0)
            st.metric("Điểm Caprini:", f"{diem_caprini} điểm")
            if diem_caprini <= 1:
                dg_cap = "Nguy cơ rất thấp (Khuyến khích vận động sớm)"
            elif diem_caprini == 2:
                dg_cap = "Nguy cơ thấp (Vớ ép áp lực hoặc thiết bị nén ngắt quãng)"
            elif diem_caprini in [3, 4]:
                dg_cap = "Nguy cơ trung bình (Vớ áp lực + Cân nhắc Heparin TLPT thấp - LMWH)"
            else:
                dg_cap = "Nguy cơ CAO (Bắt buộc dùng LMWH dự phòng + Vớ áp lực y khoa)"
            st.info(f"**Khuyến cáo dự phòng DVT:** {dg_cap}")

            if st.button("📋 Chèn vào Hướng dẫn điều trị", key="btn_chen_cap"):
                chen_txt = f"- Đánh giá nguy cơ thuyên tắc huyết khối (Caprini {diem_caprini} điểm): {dg_cap}."
                add_symptom_to_field("dt_cu_the", chen_txt)
                st.toast("Đã chèn khuyến cáo Caprini vào Điều trị!", icon="✅")

        # 5. CLAVIEN-DINDO
        elif "Clavien-Dindo" in calc_type:
            st.caption("Hệ thống phân độ biến chứng phẫu thuật kinh điển trong Ngoại khoa.")
            cd_choice = st.selectbox("Chọn mức độ biến chứng:", [
                "Grade I: Biến chứng nhẹ, chỉ dùng hạ sốt, giảm đau, bù dịch, thay băng",
                "Grade II: Cần điều trị thuốc đặc hiệu ngoài Grade I (Kháng sinh, truyền máu, dinh dưỡng TM)",
                "Grade IIIa: Cần can thiệp phẫu thuật/nội soi/xâm lấn nhưng KHÔNG gây mê toàn thân",
                "Grade IIIb: Cần can thiệp phẫu thuật/xâm lấn CÓ GÂY MÊ TOÀN THÂN (Mổ lại)",
                "Grade IVa: Biến chứng đe dọa tính mạng gây suy 1 cơ quan (ICU)",
                "Grade IVb: Suy đa cơ quan (Multiorgan dysfunction)",
                "Grade V: Bệnh nhân tử vong"
            ], key="cd_choice_val")

            grade_label = cd_choice.split(":")[0]
            st.warning(f"**Phân loại:** {cd_choice}")

            if st.button("📋 Chèn vào Chẩn đoán xác định / Tiên lượng", key="btn_chen_cd"):
                chen_txt = f"Biến chứng hậu phẫu theo Clavien-Dindo: {cd_choice}"
                add_symptom_to_field("tien_luong", chen_txt)
                st.toast("Đã chèn phân loại Clavien-Dindo!", icon="✅")

        # 6. SHOCK INDEX
        elif "Shock Index" in calc_type:
            st.caption("Nhận diện sớm sốc mất máu / giảm thể tích: SI = Mạch / HA tâm thu.")
            c_si1, c_si2 = st.columns(2)
            with c_si1:
                mach_in = st.number_input("Mạch (ck/phút)", value=int(st.session_state.get("sh_mach", 80) or 80), min_value=20, max_value=250, key="si_mach")
            with c_si2:
                ha_raw = str(st.session_state.get("sh_ha", "120/80")).split("/")[0]
                ha_tam_thu = st.number_input("HA tâm thu (mmHg)", value=int(ha_raw) if ha_raw.isdigit() else 120, min_value=30, max_value=250, key="si_ha")

            if ha_tam_thu > 0:
                si = round(mach_in / ha_tam_thu, 2)
                st.metric("Chỉ số Shock Index (SI):", f"{si}")
                if si < 0.7:
                    dg_si = "Bình thường (Huyết động ổn định)"
                    st.success(dg_si)
                elif si < 0.9:
                    dg_si = "Cận ngưỡng / Theo dõi sát mạch, huyết áp"
                    st.info(dg_si)
                elif si <= 1.1:
                    dg_si = "⚠️ Nguy cơ sốc nhẹ / Mất bù huyết động sớm"
                    st.warning(dg_si)
                else:
                    dg_si = "🚨 SỐC NẶNG / Giảm thể tích tuần hoàn cấp (Cần bù dịch & hồi sức khẩn cấp)"
                    st.error(dg_si)

                if st.button("📋 Chèn vào Khám toàn thân", key="btn_chen_si"):
                    chen_txt = f"- Tỷ số sốc (Shock Index): SI = {si} ({dg_si})"
                    add_symptom_to_field("kham_toan_than", chen_txt)
                    st.toast("Đã chèn Shock Index!", icon="✅")
